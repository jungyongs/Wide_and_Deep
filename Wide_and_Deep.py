...
def parse_args():
    ...
    parser.add_argument('--model_type', default='wide_and_deep',
                        help='wide, deep or both.')
...
def wide(train_df, test_df):
    ...
    if model_type == 'wide':
        ...
        w = Dense(y_train.shape[1], activation='sigmoid')(wide_inp)

        wide = Model(wide_inp, w)
        wide.compile(optimizer='Adam', loss='binary_crossentropy', metrics='accuracy')
        wide.fit(X_train, y_train, epochs=10, batch_size=64)
        results = wide.evaluate(X_test, y_test)

        print("wide model accuracy:", results[1])
...
def deep(train_df, test_df):
    ...
    if model_type == 'deep':
        ...
        d = Dropout(0.5)(d) 
        d = Dense(y_train.shape[1], activation='sigmoid')(d)

        deep = Model(inp_layer, d)
        deep.compile(optimizer='Adam', loss='binary_crossentropy', metrics='accuracy')
        deep.fit(X_train, y_train, epochs=10, batch_size=64)
        results = deep.evaluate(X_test, y_test)

        print("deep model accuracy:", results[1])
...
if __name__ == '__main__' : 
    ...
    if model_type == 'wide':
        wide(train_df, test_df)
    elif model_type == 'deep':
        deep(train_df, test_df)
    else:
